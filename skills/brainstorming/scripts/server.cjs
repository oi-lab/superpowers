...
function readSuperpowersVersion() {
  ...
  for (const manifest of manifests) {
    try {
      ...
      // Manifest missing or unparsable; try the next one.
    }
  }
...
function brandMarkup() {
  // Text-only branding. No remote asset is ever loaded: the visual companion
  // makes zero outbound network requests, so the page cannot phone home.
